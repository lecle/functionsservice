    ...
    container.getService('MONGODB').then(function(service) {
        ...
        service.send('findOne', {collectionName : req.session.appid, query : {where : {_className : '_Functions', name : functionName}}}, domain.bind(function(err, doc) {
            ...
            function addLog(message, data) {
                ...
                service.send('update', {collectionName : req.session.appid, query : {where : {objectId : doc.data.objectId}}, data : { $addToSet : { log : { message : message }}}}, function(err, doc) {
                ...
                if(logCnt > 1000) {
                    ...
                    service.send('update', {collectionName : req.session.appid, query : {where : {objectId : doc.data.objectId}}, data : { $pop : { log : -1}}}, function(err, doc) {
